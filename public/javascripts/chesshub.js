$( document ).ready(function() {
    ...
    socket.on('received', function (url) {
        var cur_url = window.location.href;
        if(!cur_url.includes(token)){
            $('#gameholdUrl').attr("href", url);
            $('#gameholdUrlPopup').modal({
                keyboard: false,
                backdrop: 'static'
            });
        }
	...
	/*
        ...
        socket.on('wait', function () {
            var url = "game/" + token + "/" + opponentSide;
            $('#gameUrl').html(url);
            $('#gameUrlPopup').modal({ // show modal popup to wait for opponent
                keyboard: false,
                backdrop: 'static'
            });
            socket.emit('game_hosted', url);
        });
